...
The example configuration contains four independent query slots:
...
Replace each `rawSql` value with your own Athena SQL. The project does not
assume table names, columns, or business rules.
...
```json
{
        ...
        "model": {
                "refId": "A",
                "format": "table",
                "rawSql": "SELECT date, SUM(cost) AS cost FROM aws_costs WHERE $__timeFilter(time) GROUP BY date ORDER BY date"
        }
}
```

The `model` object is sent to Grafana's `/api/ds/query` endpoint. Use the
query model generated by the Grafana panel for the selected Athena datasource.
In particular, copy the Athena panel's query model if it contains additional
Athena-specific fields. SQL,
